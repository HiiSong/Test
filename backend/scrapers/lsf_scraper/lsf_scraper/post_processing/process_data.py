...
LECTURE_DATA = os.path.abspath(os.path.join(
    backend_directory, "scrapers", "lsf_scraper", "lecture_results.json"))  # 爬取的原生的课程数据
OUTPUT_FILE = os.path.abspath(
    os.path.join(backend_directory, "scrapers", "lsf_scraper", "lsf_scraper", "Data", "post_processed_lectures.json"))  # 最后得到的处理后的课程数据
STUDY_PROGRAMS_FILE = os.path.abspath(os.path.join(
    backend_directory, "scrapers", "study_programs.json"))  # 爬取的专业数据
...
class ProcessLsfData:
    ...
    def process_timetable_of_subject(self, subject):  # 处理课程的时间表
        processed_timetable = []
    ...
    def run(self):
        ...
        with io.open(LECTURE_DATA, encoding='utf8') as json_file:
            ...
            categories_dict = {}
            subjects_list = []  # 课程列表
            einzeltermine_list = []  # 这个到底表示的是什么？
            studyprogram_list = []  # 专业列表
            ...
            merged_lectures = self.merge_lectures_with_same_id(  # 保证课程不重复
