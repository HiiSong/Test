...
pp = pprint.PrettyPrinter(indent=4)  # 格式化漂亮打印。indent (默认 1) 指定为每个嵌套级别添加的缩进量


# clears existing data in a file and creates it if it doesn't exist 清空文件/创建一个空文件
...
def run(config, insight_url, e3_url):
    ...
    lsf_data = os.path.abspath(os.path.join(
        backend_directory, config['scraped_lsf_data_directory']))  # 抓取的lsf数据json文件
    lsf_data_post_processed = os.path.abspath(os.path.join(
        backend_directory, config['post_processed_lsf_data_directory']))  # 处理后的lsf数据json文件
    vdb_data = os.path.abspath(os.path.join(
        backend_directory, config['scraped_vdb_data_directory']))  # 抓取的vdb数据json文件
    vdb_data_post_processed = os.path.abspath(os.path.join(
        backend_directory, config['post_processed_vdb_data_directory']))  # 处理后的vdb数据json文件
    ...
    merged_data_directory = os.path.abspath(os.path.join(
        backend_directory, config['merged_data_directory']))  # 合并后的数据
    ...
    lsf_scraper_directory = os.path.abspath(os.path.join(
        backend_directory, config['lsf_scraper_directory']))  # lsf_scraper路径
    vdb_scraper_directory = os.path.abspath(os.path.join(
        backend_directory, config['vdb_scraper_directory']))  # vdb_scraper路径

    # 1. run both scrapers: lsf_scraper for LSF data and vdb_scraper for Vorlesungsdatenbank data
    os.chdir(lsf_scraper_directory)  # os.chdir() 方法用于改变当前工作目录到指定的路径
    subprocess.call(
        f"scrapy crawl main -a url=\"{insight_url}\" -o lecture_results.json", shell=True)  # 似乎是在命令行输入了引号里的这一串命令从而调用外部程序，即运行这两个scrapers(爬虫)
    ...
    LsfDataProcessing = ProcessLsfData()  # lsf数据处理类
    LsfDataProcessing.run()

    VdbDataProcessing = ProcessVdbData()  # vbd数据处理类
